...
if(isset($_POST['Ville']) &&
    ...
    if(empty($ville)){
        ...
        header("Location: ../../admin/create/pension?pen=$ville&error=$em");
        ...
        if (isset($_FILES['PP'])) {
            ...
            if($error === 0){
                ...
                if (in_array($img_ex_lc, $allowed_exs)) {
                    ...
                    header("Location: ../../admin/create/pension?error=$em");
                    exit;
                }
        ...
        if (isset($new_img_name)) {
            ...
            $stmt->execute([$ville, $description, $adresse, $responsable, $telephone, $img_bdd]);
            $stmt->closeCursor();
        }else {
            # CREE UN NOUVELLE UTILISATEUR SANS L'IMAGE
            $img_default = 'assets/uploads/pension/pension-default.jpg';
            $sql = "CALL addPension(?,?,?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$ville, $description, $adresse, $responsable, $telephone, $img_default]);
            $stmt->closeCursor();
        }

        # MESSAGE DE REUSSITE + REDIRECTION
        $sm = "Nouvelle Pension Cree";
